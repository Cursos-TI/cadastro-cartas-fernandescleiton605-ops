#include <stdio.h>
// Desafio Super Trunfo - Países
// Tema 1 - Cadastro das cartas
// Objetivo: No nível novato você deve criar as cartas representando as cidades utilizando scanf para entrada de dados e printf para exibir as informações.
int main() {
  printf("Super Trunfo\n");
  // Área para definição das variáveis para armazenar as propriedades das cidades
  // Cadastros das cartas inicial
  char pais [50], country [50];
  char estado [50], ufb [50];
  char codigo [50], cod [10];
  char nome [50], city [50];
  unsigned long int populacao, habitantes;
  float area, territorio;
  float produto, bruto;
  int pontos, turistico;
  float densidade, densidade1;
  float pibpc, pibpc1;
  float podersuper;
  float superpoder;
  // Área para entrada de dados
  //Realize os cadastros que são necessários a carta 1 do jogo
  printf("Carta 1\n");
  printf("Cadastre um código para a carta:\n", codigo);
  scanf("%s", &codigo);
  printf("Cadastre o País:\n", pais);
  scanf("%s", &pais);
  printf("Cadastre o Estado:\n", estado);
  scanf("%s", &estado);
  printf("Cadastre o nome da cidade:\n", nome);
  scanf("%s", &nome);
  printf("Cadastre população:\n", populacao);
  scanf("%d", &populacao);
  printf("Cadastre área da cidade:\n", area);
  scanf("%f", &area);
  printf("Cadastre o PIB:\n", produto);
  scanf("%f", &produto);
  printf("Cadastre nº pontos turísticos:\n", pontos);
  scanf("%d", &pontos);
  //Cadastros para a carta 2
  printf("\n");
  printf("Carta 2\n");
  printf("Cadastre um código para a carta\n", cod);
  scanf("%s", &cod);
  printf("Cadastre o País:\n", country);
  scanf("%s", &country);
  printf("Cadastre o Estado:\n", ufb);
  scanf("%s", &ufb);
  printf("Cadastre o nome da cidade:\n", city);
  scanf("%s", &city);
  printf("Cadastre população:\n", habitantes);
  scanf("%d", &habitantes);
  printf("Cadastre área da cidade:\n", territorio);
  scanf("%f", &territorio);
  printf("Cadastre o PIB:\n", bruto);
  scanf("%f", &bruto);
  printf("Cadastre nº pontos turísticos:\n", turistico);
  scanf("%d", &turistico);
  //Cálculos de densidade demográfica e PIB per capita
  densidade = populacao / area;
  pibpc = produto / populacao;
  superpoder = (float) populacao + (float) turistico + area + produto + pibpc;
  // Área para exibição dos dados da cidade
  //Verifique a aparencia de suas cartas
  //Exibição Carta 1
  printf("\n");
  printf("Carta 1\n");
  printf("Código:%s\n", codigo);
  printf("País:%s\n", pais);
  printf("Estado:%s\n", estado);
  printf("Cidade:%s\n", nome);
  printf("População em milhões de habitantes:%d\n", populacao);
  printf("Área em km²:%.2f\n", area);
  printf("PIB:%.2f\n", produto);
  printf("Número de pontos turísticos:%d\n", pontos);
  printf("Densidade populacional: %.2f hab/km²\n", densidade);
  printf("PIB per capita: %.2f reais\n", pibpc);
  printf("Super Poder: %.2f\n", superpoder);
  printf("\n");
  //Cálculos de densidade demográfica e PIB per capita
  densidade1 = habitantes / territorio;
  pibpc1 = bruto / habitantes;
  podersuper = (float) habitantes + (float) pontos + territorio + bruto + pibpc1;
  //Exibição Carta 2
  printf("\n");
  printf("Carta 2\n");
  printf("Código:%s\n", cod);
  printf("País:%s\n", country);
  printf("Estado:%s\n", ufb);
  printf("Cidade:%s\n", city);
  printf("População em milhões de habitantes:%d\n", habitantes);
  printf("Área em km²:%.2f\n", territorio);
  printf("PIB:%.2f\n", bruto);
  printf("Número de pontos turísticos:%d\n", turistico);
  printf("Densidade populacional: %.2f hab/km²\n", densidade1);
  printf("PIB per capita: %.2f reais\n", pibpc1);
  printf("Super Poder: %.2f\n", podersuper);
  printf("\n");
  printf("\n### Resultados do jogo ###\n");
//Comparações entre as Cartas 1 e Carta 2
//Comparativo População
  if(populacao == habitantes)
  {
    printf("\nPopulação: Empatou!\n");
  } else if(populacao > habitantes)
  {
    printf("População: Carta 1 Venceu! (1)\n");
  } else {
    printf("População: Carta 2 Venceu! (0)\n");
  }
//Comparação Área
  if(area == territorio)
  {
    printf("Área: Empatou!\n");
  } else if(area > territorio)
  {
    printf("Área: Carta 1 Venceu! (1)\n");
  } else {
    printf("Área: Carta 2 Venceu! (0)\n");
  }
//Comparação PIB
  if(produto == bruto)
  {
    printf("PIB: Empatou!\n");
  } else if(produto > bruto)
  {
    printf("PIB: Carta 1 Venceu! (1)\n");
  } else {
    printf("PIB: Carta 2 Venceu! (0)\n");
  }
//COmparação Número de pontos turisticos
  if(pontos == turistico)
  {
    printf("Pontos turísticos: Empatou!\n");
  } else if(pontos > turistico)
  {
    printf("Pontos turísticos: Carta 1 Venceu! (1)\n");
  } else {
    printf("Pontos turísticos: Carta 2 Venceu! (0)\n");
  }
//Comparação Densidade populacional
  if(densidade == densidade1)
  {
    printf("Densidade: Empatou!\n");
  } else if(densidade < densidade1)
  {
    printf("Densidade: Carta 1 Venceu! (1)\n");
  } else {
    printf("Densidade: Carta 2 Venceu! (0)\n");
  }
//Comparação PIB per capita
  if(pibpc == pibpc1)
  {
    printf("PIB per capita: Empatou!\n");
  } else if(pibpc > pibpc1)
  {
    printf("PIB per capita: Carta 1 Venceu! (1)\n");
  } else {
    printf("PIB per capita: Carta 2 Venceu! (0)\n");
  }
//Comparação Super Poder
  if(superpoder == podersuper)
  {
    printf("Super Poder: Empatou!\n");
  } else if(superpoder > podersuper)
  {
    printf("Super Poder: Carta 1 Venceu! (1)\n");
  } else {
    printf("Super Poder: Carta 2 Venceu! (0)\n");
  } 
  printf("\n");
  return 0;
} 